  ...
  const handleLogout = () => {
    ...
    if(user){
      if (fotoPerfil !== null || fotoPerfil != " " ) {
        const items = (
        <><Card onClick={()=> redirectUser()}>{fotoPerfil ? <><><p><Avatar src={baseurl+fotoPerfil}></Avatar> {userName} {userLastName}</p></></> :<><p><Avatar size={64} style={{ background: color }}>{firstLetter}</Avatar>{userName} {userLastName}</p></>}</Card>
        ...
        return (
              ...
              <Avatar size={64} src={baseurl+fotoPerfil} />
